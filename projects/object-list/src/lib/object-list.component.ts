...
export class ObjectListComponent implements OnInit, OnChanges {
  ...
  changeOrderField(field: TableFieldModel): void {
    ...
    this.sorting = new TableSortModel(field.getFieldId(), !this.sorting.getDirection());
    this.orderData();
  }

  isSortField(field: TableFieldModel): boolean {
    return this.sorting.getFieldId() === field.getFieldId();
  }
  ...
  getRowFieldValue(row, field) {
    const value = row[field.getValueFunctionName()]();
  ...
  getRowFieldClass(row, field): string {
    const classFunctionName = field.getClassesFunctionName();
